...
export function HoverContainer({
  ...
  dbImport?: boolean;
  manageData?: boolean;
  handleUseDBFiles?: (useDBFiles: boolean) => void;
    ...
    <>
      {dbImport && (
        ...
        >
          ...
          >
            <button
              onClick={() => {
                if (!handleUseDBFiles) return;
                handleUseDBFiles(true);
              }}
      ...
      {manageData && (
        <div
          onMouseEnter={() => handleHover(true)}
          onMouseLeave={() => handleHover(false)}
          className=" hover:bg-indigo-200 hover:text-black border-white  absolute bottom-full -translate-y-0.5 border-2 text-white px-2 py-2 border-b-0 right-5 rounded-t-xl flex gap-2 h-[clamp(2rem,3vmin,8rem)] items-center"
          onClick={() => {
            if (!setIsConfirming) return;
